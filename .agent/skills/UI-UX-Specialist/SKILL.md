...
* **Global Typography Text Scaling**: Inject `MediaQuery(data: mediaQueryData.copyWith(textScaler: TextScaler.linear(settings.textSize / 16.0)))` in `MaterialApp.builder` to dynamically scale font typography preferences across the entire widget tree.
