...
- **📲 SMS Auto-Import**: Automatically reads bank SMS messages (Sri Lankan banks) and creates transactions — even while the app is in the background. Promotional, cancelled, due-reminder, and duplicate messages are silently skipped; reversals/refunds automatically delete the original expense.
- **📋 SMS Sender Whitelist**: Add non-bank senders (e.g. KOKO, FriMi) from Settings → Financial Manager → SMS Sender Whitelist to include them in auto-import. Banks are always included by default.
- **🛡️ Secure Backup**: Complete data export (Notes + Finances + Categories + SMS Whitelist + Settings) and automatic Google Cloud Backup support. Restores from any previous backup version.
